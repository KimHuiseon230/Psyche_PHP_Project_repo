...
if (isset($_POST["mode"]) && $_POST["mode"] === "delete") {
	...
	if (!$rowcount) {
		...
		$sql = "INSERT INTO `event_ripple` VALUES (null,'$q_parent','$q_userid','$q_username', '$q_usernick','$q_content','$regist_day')";
		$stmt = $conn->prepare($sql);
		$row = $stmt->fetch();
		$result = $stmt->execute();
		...
		echo "<script>location.href='./event_view.php?num=$parent&page=$page&hit=$hit';</script>";
	} //end of if rowcount
	...
	$sql = "DELETE FROM `event_ripple` WHERE num=$q_num";
	$stmt = $conn->prepare($sql);
	$row = $stmt->fetch();
	$result = $stmt->execute();
	if (!$result) {
