...
if (isset($_POST["mode"]) && $_POST["mode"] === "delete") {
    ...
    $sql = "delete from `product_board` where num = :num";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':num', $num);
    $stmt->execute();
    echo "
    ...
    $sql = "insert into product_board (id, name, subject, content, regist_day, hit,  file_name, file_type, file_copied) ";
    ...
    $sql = "update `product_board` set subject=:subject, content=:content,  file_name=:upfile_name, file_type=:upfile_type, file_copied=:copied_file_name";
    $sql .= " where num=$num";
    $stmt = $conn->prepare($sql);
    ...
    if (!$rowcount) {
        ...
        $sql = "INSERT INTO `product_board_ripple` VALUES (null,'$q_parent','$q_userid','$q_username', '$q_usernick','$q_content','$regist_day')";
        $stmt = $conn->prepare($sql);
        $row = $stmt->fetch();
        $result = $stmt->execute();
    ...
    $sql = "DELETE FROM `product_board_ripple` WHERE num=:num";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':num', $num);
    $result = $stmt->execute();
    $row = $stmt->fetch();
